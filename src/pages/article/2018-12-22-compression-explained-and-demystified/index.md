...
Compression can simply be explained as an “automated volume fader” or dynamic tool, but does that really clarify how to use it for you? To muddy the waters further, a compressor set the wrong way might do exactly the opposite of what you want it to do.

A big part of learning how to use compression is a lot of listening and experimenting. Hopefully after this article you’ll have at least a conceptually understanding of exactly how the compressor is affecting a sound. 

##Let's Focus Up

To make things simpler, we’re going to compress one thing super hard so it’s very clear how the compressor is working and how the compressor parameters were used to achieve the result. 
...
All the higher peaks are right at the beginning and the whole snare hit happens very very fast. 
Keep that in mind as we start going through the controls. 
##The Compressor's Controls (or Parameters)

###Threshold 
Threshold is the point at which the compressor starts compressing. If you look at your compressors gain reduction meter and see no gain reduction, then the whole signal is below the threshold. Turn down the threshold until you see gain reduction on the meters. 
...
###Ratio 
Factually speaking the ratio is how much the signal is getting compressed above the threshold. The higher the ratio, the more compression. 
...
###Attack 
Attack, factually speaking, is a rate at which gain reduction is applied after the signal crosses the threshold.
...
If you have a very short sound, like a snare sidestick, with a long attack time, the compressor might not be doing anything at all to compress the signal. That is...
...
The first sidestick hits the compressor, it goes above the threshold telling the compressor it should start applying gain reduction. Now let's say you set the attack to 100ms and the release to 5000ms. 
...
Jesus I hope that's not too boring. 
...
In order to get a bigger body we are reducing the dynamic range of the snare hit. All of the very loud parts of the snare take place basically in first 3ms. In other words, REALLY FRIGGEN QUICKLY.
...
If you look at the picture of the "smack" snare, i.e. the uncompressed snare. You'll see there is a sharp drop in level very quickly. 
...
This is a crazy amount of compression, but sometimes exploring the extremes helps people to hear the subtleties. 
...
Take a guess at around what the attack should be to go from the fat thwack on the left to the more smack sound on the right. Hint: Look back at how long the snare hit is and how long the initial peaks were.
...
So you can see the attack is around 10ms. Enough to let the initial transients through and then start reducing the body of the snare hit again. The release doesn't necessarily need to be that long, but it needs to be long enough that the compressor is still reducing the body to some extent through the end of the hit. 


###Back to the beginning
Now imagine you wanted to make the original, uncompressed snare fatter, but you set your compressor up with an attack time too long. You might actually INCREASE the dynamic range doing the opposite of what you wanted. 
